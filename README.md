...
| **Caching** | Verify that static page resources are served from CloudFront cache, by checking the following header ```X-Cache: Hit from cloudfront```. If the result is a miss, reload the page and verify again. Verify that the html for the home page also results in a cache hit, when no user is logged. Log in (user: Joud, pwd: demo), reload the home page, and verify that the logged experience is not cached.|
